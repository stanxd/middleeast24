...
import { Mail, FileText, Users, Newspaper, Shield } from 'lucide-react';
...
interface AdminStatsProps {
  ...
  articles?: any[];
  exclusiveSourceSubmissions?: any[];
}
...
const AdminStats: React.FC<AdminStatsProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Exclusive Sources</CardTitle>
          <Shield className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{exclusiveSourceSubmissions?.length || 0}</div>
          <p className="text-xs text-muted-foreground">
            {exclusiveSourceSubmissions?.filter(s => s.status === 'unread').length || 0} unread
          </p>
        </CardContent>
      </Card>
    </div>
